...
class CsvLogger:
    ...
    def log(self, row, step):
        ...
        if self.file is None:
            # Check if file exists and we are appending
            file_exists = os.path.exists(self.path) and self.mode == 'a'
            self.file = open(self.path, self.mode)
            
            if self.header is None:
                self.header = [k for k, v in row.items() if not isinstance(v, self.disallowed_types)]
                # Write header only if we are not appending to an existing file
                if not file_exists:
