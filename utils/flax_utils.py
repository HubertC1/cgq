...
def save_agent(agent, save_dir, epoch, max_to_keep=None):
    ...
    Args:
        ...
        epoch: Epoch number.
        max_to_keep: Maximum number of checkpoints to keep. If None, keep all.
    """
    ...
    if max_to_keep is not None:
        ...
        checkpoints = glob.glob(os.path.join(save_dir, 'params_*.pkl'))
        # Sort by epoch
        def get_epoch(path):
            try:
                return int(path.split('params_')[-1].split('.pkl')[0])
            except ValueError:
                return -1
        ...
        while len(checkpoints) > max_to_keep:
            ...
            try:
                ...
                print(f'Error removing checkpoint {to_remove}: {e}')
